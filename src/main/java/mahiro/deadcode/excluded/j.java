    ...
    public void a(FontRenderer fontRenderer, ItemStack itemStack, int n2, int n3, String string) {
        if (!itemStack.isEmpty()) {
            ...
            if (itemStack.getItem().showDurabilityBar(itemStack)) {
                ...
                GlStateManager.disableAlpha();
                GlStateManager.disableBlend();
                string2 = Tessellator.getInstance().toString();
            ...
            float f3 = f2 = (string2 = String.valueOf(Minecraft.getMinecraft().player)) == null ? 0.0f : Minecraft.getMinecraft().player.getCooldownTracker().getCooldown(itemStack.getItem(), Minecraft.getMinecraft().getRenderPartialTicks());
            if (f2 > 0.0f) {
                GlStateManager.disableLighting();
                GlStateManager.disableDepth();
                GlStateManager.disableTexture2D();
