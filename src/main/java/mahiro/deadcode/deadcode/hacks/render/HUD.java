...
import org.lwjgl.opengl.GL11;import utils.*;
...
extends Hack {
    ...
    public void b() {
        List<Hack> list = HackManager.getSortedHacks();
        int n2 = 10;
        int n3 = 0;
        int n4 = 0;
    ...
    public void c() {
        ...
        if (((Boolean)this.p.getObjectValue()).booleanValue()) {
            n10 += n8;
            string = String.format("utils.x: %s y: %s z: %s", RenderUtils.a(Float.valueOf((float)entityPlayerSP.posX), 1), RenderUtils.a(Float.valueOf((float)entityPlayerSP.posY), 1), RenderUtils.a(Float.valueOf((float)entityPlayerSP.posZ), 1));
