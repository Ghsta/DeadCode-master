...
import org.apache.commons.lang3.RandomUtils;import utils.*;
...
extends Hack {
    ...
    public MagicCarpet() {
        super("MagicCarpet", HackCategory.World);
        this.b("General");
    ...
    public void c() {
        ...
        for (EnumFacing enumFacing : EnumFacing.values()) {
            ...
            if (vec3d.squareDistanceTo(new Vec3d((Vec3i)this.e).addVector(0.5, 0.5, 0.5)) >= vec3d.squareDistanceTo(new Vec3d((Vec3i)blockPos).addVector(0.5, 0.5, 0.5)) || !eS.b(blockPos).canCollideCheck(Wrapper.INSTANCE.getWorld().getBlockState(blockPos), false) || vec3d.squareDistanceTo(vec3d2 = new Vec3d((Vec3i)blockPos).addVector(0.5, 0.5, 0.5).add(new Vec3d(enumFacing2.getDirectionVec()).scale(0.5))) > 18.0625) continue;
    ...
    public void d() {
        ...
        for (EnumFacing enumFacing : EnumFacing.values()) {
            ...
            BlockPos blockPos = this.e.offset(enumFacing);
            EnumFacing enumFacing2 = enumFacing.getOpposite();
            if (!eS.b(blockPos).canCollideCheck(eS.a(blockPos), false) || vec3d.squareDistanceTo(vec3d2 = new Vec3d((Vec3i)blockPos).addVector(0.5, 0.5, 0.5).add(new Vec3d(enumFacing2.getDirectionVec()).scale(0.5))) > 36.0) continue;
